...
class ImageDownloaderApp(QWidget):
    ...
    def create_download_view(self):
        ...
        self.label_csv = QLabel("Choose CSV file:")
        layout.addWidget(self.label_csv)

        self.btn_select_csv = self.create_button("Choose CSV with EAN and URL")
        self.btn_select_csv.clicked.connect(self.select_csv)
        layout.addWidget(self.btn_select_csv)

        self.label_folder = QLabel("Choose Target Folder:")
        layout.addWidget(self.label_folder)

        self.btn_select_folder = self.create_button("Choose Folder")
        self.btn_select_folder.clicked.connect(self.select_folder)
        layout.addWidget(self.btn_select_folder)

        self.btn_download = self.create_button("Run", enabled=False)
    ...
    def select_csv(self):
        ...
        file_path, _ = file_dialog.getOpenFileName(self, "Choose CSV File", "", "CSV Files (*.csv)")
        if file_path:
    ...
    def select_folder(self):
        ...
        folder_path = folder_dialog.getExistingDirectory(self, "Choose Target Folder")
        if folder_path:
            self.target_folder = folder_path
            self.label_folder.setText(f"Folder: {folder_path}")
            self.update_download_button()
    ...
    def download_images(self):
        if not self.csv_file or not self.target_folder:
            self.result_text.append("❌ Requirements are missing (CSV file or target folder).")
            return
        ...
        with open(self.csv_file, "r", encoding="utf-8") as file:
            ...
            if len(rows) < 2:
                self.result_text.append("❌ CSV file is empty or invalid..")
                return
            ...
            for i, row in enumerate(rows[1:], 1):
                ...
                try:
                    ...
                    self.result_text.append(f"✅ {ean}.jpg saved.")
